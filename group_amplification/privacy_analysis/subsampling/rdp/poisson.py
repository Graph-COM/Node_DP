...
def _rdp_tight_pos_neg_quadrature_gaussian(
        alpha: int, base_mechanism: GaussianMechanism, subsampling_rate: float,
        insertions: int, deletions: int, in_neg_prob: int, del_neg_prob: int, eval_params: dict, constant_sensitivity=False) -> float:
    """Tight group privacy amplification for Poisson subsampled Gaussian via quadrature.
    ...
    #w_p = binom.pmf(np.arange(deletions + 1), deletions, subsampling_rate)
    #w_q = binom.pmf(np.arange(insertions + 1), insertions, subsampling_rate)
    w_del = [1-del_neg_prob, del_neg_prob]
    w_in = [1-in_neg_prob, in_neg_prob]
    w_p = [binom.pmf(i, deletions, subsampling_rate) * w_del[j] for i, j in itertools.product(range(deletions+1), range(2))]
    w_q = [binom.pmf(i, insertions, subsampling_rate) * w_in[j] for i, j in itertools.product(range(insertions+1), range(2))]

    if constant_sensitivity:
        def integrand(z):
                densities_p = np.array([mpm.npdf(z, 0 if i==0 and j==0 else 1, base_mechanism.standard_deviation)
                                        for i, j in itertools.product(range(deletions+1), range(2))])

                densities_q = np.array([mpm.npdf(z, 0 if i==0 and j==0 else -1, base_mechanism.standard_deviation)
                                        for i, j in itertools.product(range(insertions+1), range(2))])
        ...
        def integrand(z):
            densities_p = np.array([mpm.npdf(z, i + 2 * j, base_mechanism.standard_deviation)
                                    for i, j in itertools.product(range(deletions + 1), range(2))])

            densities_q = np.array([mpm.npdf(z, -(i + 2 * j), base_mechanism.standard_deviation)
                                    for i, j in itertools.product(range(insertions + 1), range(2))])
            p = w_p @ densities_p
            q = w_q @ densities_q
            ratio = p / q
    ...
    with mpm.workdps(eval_params['dps']):
        #res = mpm.log(mpm.quad(integrand, [-mpm.inf, mpm.inf])) / (alpha - 1)
        res = mpm.log(mpm.quad(integrand, [-4, 300])) / (alpha - 1) # [-4, 300] is a manually chosen interval that seems to make sense for most cases
        return float(res)
