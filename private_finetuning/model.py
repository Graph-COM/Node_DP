...
class GaLMModel(nn.Module):
    ...
    def build(
        ...
        **hf_kwargs,
        ...
        if "bert" in model_args.model_type:
            if 'lora' in model_args.model_name_or_path or 'nondp' in model_args.model_name_or_path:
                ...
                if train_args.do_train:
                    trainable_layers = [lm.encoder.layer[-1], lm.pooler]
                    trainable_params = 0
        ...
        elif "sllama" in model_args.model_type:
            lm = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path)
        else:
            raise NotImplementedError
...
class DPGaLMModel(GaLMModel):
    def forward(self, query, key, neg_key=None):
        ...
        if self.training:
            if self.train_args.dp_type == 'node':
                q_id, k_id, neg_id = query['input_ids'], key['input_ids'], neg_key['input_ids']
                _, nid, freq = torch.unique(torch.cat([q_id, k_id, neg_id]), dim=0, return_inverse=True, return_counts=True)
                if len(torch.unique(freq)) != 1:
                    ratio = freq[nid]
                    ratio = 1 / torch.cat([ratio[:2*batch_size].view(2,-1), ratio[2*batch_size:].view(batch_size, -1).t()]).max(dim=0).values
                    self.train_args.dp_optimizer._set_per_sample_clip_factor(ratio)
