...
def compute_metrics_mia(evalpred: EvalPrediction):
    '''
    output of evalpred: 
        head embedding [n, 4096], 
        tail embedding [n, 4096],
        dot product by batches [n, 256], 
        labels [n]
    '''
    scores, labels = evalpred.predictions[-2], evalpred.predictions[-1]
    # preds = scores[np.arange(len(labels)), labels]
    loss = torch.nn.functional.cross_entropy(torch.tensor(scores), torch.tensor(labels), reduction='none')
    cosine = torch.nn.functional.cosine_similarity(torch.tensor(evalpred.predictions[0]), torch.tensor(evalpred.predictions[1]), dim=1)
    ...
    return {'preds': loss, 'loss': loss, 'cos': cosine, "prc": prc, "mrr": mrr}
