            ...
            for epoch in range(epochs_trained, num_train_epochs):
                ...
                with BatchMemoryManager(data_loader=epoch_iterator,
                    ...
                    for step, inputs in enumerate(memory_safe_data_loader):
                        # print(f"physcial batch_size {len(inputs[0]['input_ids'])}")
                        ...
                        if (
                            ...
                            if optimizer_was_run:
                                ...
                                if not isinstance(self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                                    ...
                                    # print('LR at DPOptimizer: {}'.format([g['lr'] for g in self.optimizer.param_groups]))
                                    # print('LR at AcceleratedOptimizer: {}'.format([g['lr'] for g in self.optimizer.original_optimizer.param_groups]))
                                    # print('LR at original Optimizer: {}'.format([g['lr'] for g in self.optimizer.original_optimizer.optimizer.param_groups]))

                                # model.zero_grad()
                                self.state.global_step += 1
                                self.state.epoch = epoch + (step + 1 + steps_skipped-steps_batch_accum) / steps_in_epoch
                                self.control = self.callback_handler.on_step_end(args, self.state, self.control)
                                if self.state.global_step < self.args.start_eval*(16//self.args.per_device_train_batch_size):
