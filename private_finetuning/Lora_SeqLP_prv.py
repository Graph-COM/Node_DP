...
def get_lora_model(model_checkpoints, model, rank=4, alpha=16, lora_dropout=0.1, bias='none'):
    if 'bert' in model_checkpoints:
        peft_config = LoraConfig(
            ...
            # layers_to_transform = [4,5,6,7,8,9,10,11],
            # use_dora=True,
        )
...
def main():
    ...
    # args.lora_rank = wandb.config.lora_rank
    # args.lora_alpha = wandb.config.lora_alpha
    # args.lora_dropout = wandb.config.lora_dropout
    ...
    if model_args.pdebug:
        ...
        display_name = f"{model_args.model_type}_{'edp' if training_args.dp_type=='edge' else 'ndp'}_{data_args.dataset_name}_b{training_args.per_device_train_batch_size}_n{training_args.neg_k}_r{training_args.lora_rank}"
        wandb.init(entity="xxx", project="yyy", name=display_name, tags=["zzz"])
